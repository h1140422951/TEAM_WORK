...
namespace 电子族谱管理系统
{
    public partial class 查询管理界面 : Form
    {
        ...
        private void button4_Click_1(object sender, EventArgs e)
        {
            ...
            String str2 = "SELECT Member_ID,Father_ID,Genealogy_ID,name,sex,birth,death,birthplace,address,spouse_name,spouse_state,number  FROM [Member] WHERE name='" + str1 + "'";
            gene r1 = new gene();   //我写的那个用来连接数据库的类是ResM，所以用其创建对象；
            DataTable d1 = new DataTable();   //因为查询是会有结果返回的，所以需要用DataTable的对象来承接返回的结果。
            d1 = r1.ExecuteQuery(str2);
            if (d1 != null && d1.Rows.Count > 0)
            ...
            {
                MessageBox.Show("查不到此成员，请重新输入成员姓名", "温馨提示", MessageBoxButtons.OKCancel, MessageBoxIcon.Exclamation);
            }
        }
    }
}
